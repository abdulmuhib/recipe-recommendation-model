...
def recommend_recipes(input_ingredients, k=5):
    input_ingredients = [
        ingredient.strip().lower() for ingredient in input_ingredients
    ...
    for idx in indices:
        recipe_ingredients = [
            ingredient.strip().lower() for ingredient in ingredients[idx].split(",")
        ]
            ...
            recommended_recipes.append(
                {
                    ...
                    "image_path": image_paths[
                        idx
                    ],  # Include the image path in the recommendation
                }
            )
...
def generateIngredients(input_ingredients):
    input_ingredients = [
        ingredient.strip() for ingredient in input_ingredients.split(",")
    ]
    ...
    if result:
        for recipe in result:
            ...
            net_price = (
                int(recipe["net_price"])
                if isinstance(recipe["net_price"], np.int64)
                else recipe["net_price"]
            )
...
def api_recommend_recipes():
    ...
    if data and "ingredients" in data:
        ingredients_list = data[
            "ingredients"
        ]  # Assuming 'ingredients' is a list in the JSON data
